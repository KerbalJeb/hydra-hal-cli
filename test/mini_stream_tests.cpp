...
{
  int answer = 42;
  cli << "The answer to life, the universe and every thing is: " << answer
      << cli::format::endl;

  ASSERT_EQ (
      mock_serial::ostring,
      "The answer to life, the universe and every thing is: 42\n\r\x1B[0m");
}

TEST_F (MiniStreamTests, chained_output_multi_base)
{
  int answer = 42;
  cli << "The answer to life, the universe and every thing is: 0x"
      << cli::format::hex << answer << " in hex and " << cli::format::dec
      << answer << " in decimal" << cli::format::endl;

  ASSERT_EQ (mock_serial::ostring,
             "The answer to life, the universe and every thing is: 0x2A in "
             "hex and 42 in decimal\n\r\x1B[0m");
}
...
TEST_F (MiniStreamTests, multiple_line_streams)
{
  ...
  cli << "The answer to life, the universe and every thing is: 0x"
      << cli::format::hex;
  cli << answer << " in hex and " << cli::format::dec << answer
      << " in decimal" << cli::format::endl;
...
{
  cli << cli::format::text::red << "This text is red " << cli::format::reset
      << "this text is not" << cli::format::endl;

  ASSERT_EQ (mock_serial::ostring,
             "\x1B[31mThis text is red \x1B[0mthis text is not\n\r\x1B[0m");
}
